...
void Tournament::connectAllPlayers(ServerSocket _serverSocket)
{
	...
	for (int i = nbClient; i != NB_CLIENT; i++) {
		...
		#ifdef __linux__
		if (fork() == 0) {
			char *args[4] = {"./ia", "IA", const_cast<char*>(std::to_string(_port).c_str()), NULL};
			execvp("./ia", args);
		}
		#endif
		#ifdef _WIN64
...
void Tournament::start(std::string port)
{
	...
	#ifdef __linux__
	if (fork() == 0) {
		char *args[4] = {"./server_tournament", strdup(port.c_str()), strdup(std::to_string(_id).c_str()), NULL};
		execvp("./server_tournament", args);
	}
	#endif
	#ifdef _WIN64
	STARTUPINFO si;
